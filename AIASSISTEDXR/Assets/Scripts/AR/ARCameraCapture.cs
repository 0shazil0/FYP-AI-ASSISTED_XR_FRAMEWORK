...
namespace NeuroGuideXR.AR
{
    public sealed class ARCameraCapture : MonoBehaviour
    {
        ...
        // FIX (Bug 4): Samsung Galaxy S25 Ultra delivers raw ARCore CPU images in portrait-native
        // layout. MirrorY corrects the orientation so the VLM receives an upright scene image.
        // If the backend VLM still describes the scene as mirrored, switch to MirrorX here.
        [Header("Image Capture")]
        [Tooltip("Transformation applied to the raw ARCore CPU image before JPEG encoding.\n" +
                 "MirrorY = correct for Samsung Galaxy S25 Ultra portrait mode.\n" +
                 "If scene appears mirrored in VLM responses, try MirrorX.")]
        [SerializeField] private XRCpuImage.Transformation _imageTransformation =
        ...
        public IEnumerator CaptureAndSendFrame(string messageType, string promptText, string requestMode, Action<bool, string> onCompleted)
        {
            ...
            // FIX (Samsung S25 Ultra / ARCore): The raw CPU image from TryAcquireLatestCpuImage
            // is delivered in portrait-native YUV layout. Using Transformation.None produces a
            // horizontally-mirrored / 90°-rotated image that causes the VLM to mis-read the scene.
            //
            // MirrorY is correct for Samsung Galaxy devices in portrait orientation.
            // If the VLM still describes the scene upside-down or mirrored after this fix,
            // switch to MirrorX in the Inspector using the _imageTransformation field below.
            //
            // How to test: hold the phone in front of a known reference object (e.g. a coffee mug).
            // Tap Snap. Check the backend log — it prints decode_frame shape and sample pixel.
            // Ask the VLM "describe what you see" — it should say the object right-side-up.
            var conversion = new XRCpuImage.ConversionParams
            {
                inputRect = new RectInt(0, 0, image.width, image.height),
                outputDimensions = new Vector2Int(outWidth, outHeight),
                outputFormat = TextureFormat.RGB24,
                transformation = _imageTransformation
            };
